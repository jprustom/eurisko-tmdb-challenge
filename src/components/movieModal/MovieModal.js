...
export default function MovieModal(){
    ...
    if (!selectedMovie)
        return null
    const {release_date:releaseDate,homepage,title,overview,vote_average: voteAverage,backdrop_path:backdropPath}=selectedMovie
    
    return <Modal show={selectedMovie} onHide={onHideHandler}>
                {
                    selectedMovie
                    ?<>
                        ...
                        <Modal.Body>
                            ...
                            <h6 className="my-2">Genres: {displayGenres()}</h6>
                            <p>{overview}</p>
                            <p>{voteAverage}/10</p>
                            <p>Released on {releaseDate.replaceAll("-","/")}</p>
                        </Modal.Body>
                        <Modal.Footer onClick={()=>window.open(homepage)} className={`${MovieModalStyles.NetflixLinkContainer} bg-primary`}>
                            <span className={MovieModalStyles.NetflixLink} rel="noreferrer">Watch now </span>
                        </Modal.Footer>
                        <Modal.Footer>
